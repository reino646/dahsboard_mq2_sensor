    ...
    <div class="flex h-screen bg-gray-800 " :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full overflow-y-auto bg-gradient-to-r from-red-800 via-orange-800 to-yellow-500 ">
            ...
            <main class="px-4 sm:px-6 lg:px-8 py-10">
                <div class="grid gap-6 rounded-3xl bg-white border-4 border-orange-700 p-6 sm:p-8 lg:p-10">

                    <!-- Breadcrumb -->
                    <div class="flex items-center h-10">
                        <h2 class="text-sm sm:text-base md:text-lg font-semibold truncate opacity-80">/Page/Users Management</h2>
                    </div>

                    <!-- Heading -->
                    <div>
                        <h1 class="font-semibold text-2xl sm:text-3xl md:text-4xl">Users Management</h1>
                        <p class="font-light mt-2 text-sm sm:text-base">Manage the users and set the roles the way you want!</p>
                    </div>
                    ...
                    <!-- Desktop: Table with card style -->
                    <div class="hidden md:block">
                        <div class="bg-white rounded-2xl shadow-lg p-6 overflow-x-auto border border-orange-700">
                                ...
                                <tbody class="bg-white">
                                    @if($users)
                                        @foreach($users as $uid => $user)
                                            <tr class="border-t">
                                                <td class="py-2 px-4">{{ $uid }}</td>
                                                <td class="py-2 px-4">{{ $user['email'] ?? '-' }}</td>
                                                <td class="py-2 px-4">{{ $user['name'] ?? '-' }}</td>
                                                <td class="py-2 px-4">
                                                <form method="POST" action="{{ url('/update-role/' . $uid) }}">
                                                        @csrf
                                                        <select name="role" class="text-sm rounded-3xl bg-red-600 text-white px-4 py-2 hover:opacity-80">
                                                            <option value="user" {{ ($user['role'] ?? '') === 'user' ? 'selected' : '' }}>User</option>
                                                            <option value="admin" {{ ($user['role'] ?? '') === 'admin' ? 'selected' : '' }}>Admin</option>
                                                        </select>
                                                        <button class="text-sm rounded-3xl bg-orange-500 text-white px-4 py-2 hover:opacity-80 mt-2" type="submit">Update</button>
                                                    </form>
                                                </td>
                                                <td class="py-2 px-4">
                                                    <form method="POST" action="{{ url('/delete-user/' . $uid) }}" onsubmit="return confirm('Are you sure to delete this user?');" class="mt-2">
                                                        @csrf
                                                        <button class="text-sm rounded-3xl bg-red-500 text-white px-4 py-2 hover:opacity-80 w-full" type="submit">Delete</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr><td colspan="5" class="py-4 px-4 text-center">No user found</td></tr>
                                    @endif
                                </tbody>
                            </table>
                        </div>
                    ...
                    <!-- Mobile: Card style per user -->
                    <div class="md:hidden space-y-4">
                        @if($users)
                            @foreach($users as $uid => $user)
                                <div class="rounded-2xl border border-orange-700 p-4 bg-white shadow-md">
                                    <div class="text-sm text-gray-600">UID:</div>
                                    <div class="font-medium mb-2 break-words">{{ $uid }}</div>

                                    <div class="text-sm text-gray-600">Email:</div>
                                    <div class="mb-2 break-words">{{ $user['email'] ?? '-' }}</div>
                                    ...
                                    <form method="POST" action="{{ url('/update-role/' . $uid) }}" class="mt-3">
                                        @csrf
                                        <select name="role" class="w-full mb-2 text-sm rounded-3xl bg-red-600 text-white px-4 py-2 hover:opacity-80">
                                            <option value="user" {{ ($user['role'] ?? '') === 'user' ? 'selected' : '' }}>User</option>
                                            <option value="admin" {{ ($user['role'] ?? '') === 'admin' ? 'selected' : '' }}>Admin</option>
                                        </select>
                                        <button class="w-full text-sm rounded-3xl bg-orange-500 text-white px-4 py-2 hover:opacity-80" type="submit">Update</button>
                                    </form>
                                    <form method="POST" action="{{ url('/delete-user/' . $uid) }}" onsubmit="return confirm('Are you sure to delete this user?');" class="mt-2">
                                        @csrf
                                        <button class="w-full text-sm rounded-3xl bg-red-500 text-white px-4 py-2 hover:opacity-80" type="submit">Delete</button>
                                    </form>
                                </div>
                            @endforeach
                        @else
                            <div class="text-center text-gray-600">No user found</div>
                        @endif
                    </div>
